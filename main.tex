...
所以建议在数学公式中使用 \lvert 和 \rvert 来表示绝对值符号，例如 $\lvert x \rvert$。下面是两个示例表格：
...
\begin{table}[htbp]
    ...
    \begin{tabular}{l c c r}
        ...
        \textbf{Ours (Pro)} & \textbf{92.1\%}        & $\lvert \Delta \theta \rvert_2$  & \textbf{38.2} \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{table}[htbp]
    \centering
    \caption{Comparison of performance metrics across different models.}
    \label{tab:experiment_results_bad}
    \begin{tabular}{l c c r}
        \toprule
        \textbf{Model Name} & \textbf{Accuracy (\%)} & \textbf{Loss Function} & \textbf{Time (ms)} \\
        \midrule
